...
- **映射功能**：单选「向左切换桌面」或「向右切换桌面」，实现为发送系统自身的 `⌃←` / `⌃→`，因此依赖「系统设置 → 键盘 → 键盘快捷键 → 调度中心」里的「向左移动一个空间 / 向右移动一个空间」处于开启状态。Tocode 不读取也不修改该系统偏好；若它被关闭或被改键，功能会静默失效。全屏应用或只有一个桌面时系统本身也不会切换。
- 名称不能为空；名称与源快捷键在规则间各自唯一。源快捷键不能与所选功能自身的触发键相同（例如源 `⌃←` → 目标「向左切换桌面」，映射后没有任何效果），但把 `⌃←` 重映射为其他目标是允许的。
- 命中源快捷键时吞掉源 `keyDown` / `keyUp`；自动重复只吞掉不重复合成。合成事件带内部标记，既不会再次进入重映射，也不会触发双击⌘Q等既有保护。
