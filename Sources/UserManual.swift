...
enum UserManual {
    ...
    static let pages: [Page] = [
        ...
        """),
        Page(title: "目录树", body: """
        左键打开目录树后，默认回到上次展开到的文件夹（路径仍在当前根下且存在时）；否则从根目录第一层开始。悬停文件夹即可展开下级。未按修饰键时，点击文件或文件夹会把绝对路径以「路径」形式复制到剪贴板，菜单随后关闭。

        当不在根层时，菜单顶部显示当前位置面包屑，并提供「上一级」与「根目录」便于继续索引。悬停更深的子文件夹会更新下次打开的位置。

        按住 Shift 可连续点击多个条目：按点击顺序把多条「路径」一行一条写入剪贴板，且目录树保持打开。松开 Shift 后再点一次，仍只复制那一条并关闭菜单。Shift 多选只在普通复制模式下有效；按住 Option 或 Command 时仍是删除或访问。
